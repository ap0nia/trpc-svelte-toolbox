...
:::tip
`loadContext` is intended to work like `getContext`, but outside of components.

It returns a proxy that uses the query client created in `+layout.ts` without requiring `setContext` to provide it.

In components, I thought using `setContext` to set the query client and evaluate the proxy was appropriate since
it's difficult to predict where the query client might originate in different apps.
:::
...
export const load: PageLoad = async ({ parent }) => {
  ...
  return {
    greeting: trpc.loadContext.hello.fetch({ text: 'client' });
  }
}
```
